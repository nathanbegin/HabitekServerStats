...
class DeviceData(db.Model):
    ...
    record_type = db.Column(db.String(50), nullable=False) # 'sensor' or 'camera'
...
# Helper for pretty JSON responses
def pretty_response(obj):
    text = json.dumps(obj, indent=2, ensure_ascii=False)
    resp = make_response(text)
    resp.mimetype = 'application/json'
...
def webhook():
    ...
    try:
        ...
        cmd = data.get('cmd')
        if cmd == 'set_http_data':
            device_uuid = data.get('device_uuid')
            # Assuming 'data' contains sensor or camera info
            values = data.get('data')
            timestamp_str = data.get('time')
            record_type = 'sensor' # Default type

            if 'AD' in values and 'data' in values['AD'] and 'image' in values['AD']['data']:
                record_type = 'camera'
                # For camera, the relevant data is the image URL
                sensor_data = {'image': values['AD']['data']['image']}
            elif 'DI' in values: # Digital Input for sensor data
                # Extract relevant sensor data for 'DI' type
                sensor_data = {
                    'temperature': values['DI'].get('T_c'),
                    'humidity': values['DI'].get('H')
                }
                # Filter out None values
                sensor_data = {k: v for k, v in sensor_data.items() if v is not None}
            else:
                logger.warning('Unknown data format received: %s', data)
                return jsonify({'status': 'ignored', 'message': 'Unknown data format'}), 200
            ...
            db.session.commit()
            logger.info('Received data from %s (%s): %s', device_uuid, record_type, sensor_data)

            # Emit new data to all connected Socket.IO clients
            socketio.emit('new_data', {
                'device_uuid': device_uuid,
                'timestamp': timestamp.isoformat() + 'Z',
                'record_type': record_type,
                'values': sensor_data # Use 'values' key to match App.jsx expectation
            })
